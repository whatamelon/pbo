<template>
    ...
    <ItemHeightControl id="heightC"></ItemHeightControl>
...
import ItemHeightControl from "@/components/Item/ItemHeightControl";
...
export default {
    ...
    methods: {
       async filterSticky() {
        ...
        if(window.pageYOffset > curateIN) {
            heightC.classList.add("sticky2");
            buttonContainer.classList.add("sticky");
        }
        else {
            heightC.classList.remove("sticky2");
  ...
  beforeRouteLeave(to, from, next) {
    if(to.name == "model-id"||
        ...
        this.$store.dispatch("setHeightRange","");
    }
