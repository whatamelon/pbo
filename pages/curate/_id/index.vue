<template>
    <main class="cp-main">
        ...
        <div class="ci-coverUp" @click="goToCurator(Q_INFO.quserName)">
...
<div v-if="Q_INFO.quserName == '하티' || Q_INFO.quserName == '해피소민' || Q_INFO.quserName == '민송'
|| Q_INFO.quserName == '민또링'|| Q_INFO.quserName == '리엔'">
</div>
<div v-else>
        <div class="cpm-models" 
        v-show="Q_MODEL_LIST != null || Q_INFO.quratorId != 6|| Q_INFO.quratorId != 7 || 
        Q_INFO.quratorId != 8|| Q_INFO.quratorId != 9|| Q_INFO.quratorId != 10">
            <span class="cpm-title">총<span class="cpm-length"> {{ Q_MODEL_LIST.length }}</span>명의 모델이 입고 촬영한 상품들이에요.</span>
            <div class="cpm-container">
                <div class="cpm-model" v-for="model in Q_MODEL_LIST" :key="model.modelId"
                @click="goToModel(model)" v-show="model.imgFaceFile != '/models/_small.jpg'">
                    <img class="cpm-modelPic" :src="IMAGE_URL+model.imgFaceFile"/>
                    <span class="cpm-modelName">{{ model.name }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="tc-container">상품수 <span class="tc-number">{{ modiCount(TOTAL_CNT) }}</span>개</div>
    ...
    <ItemCatPopUp v-if="Q_CAT_LIST != null"></ItemCatPopUp>
    <ItemCatPopUp2 v-else></ItemCatPopUp2>
...
import ItemCatPopUp2 from "@/components/Item/ItemCatPopUp2";
...
export default {
  ...
  computed: {
    ...
    goToCurator(i) {
        console.log(i);
        if(i == "민송") {
                this.$amplitude.getInstance().logEvent("click curatorThumbnail", {curatorName: '민송'});
                this.quratorId == '6';
            this.$router.push('/qurator/6');
        } else if(i == "하티") {
                this.$amplitude.getInstance().logEvent("click curatorThumbnail", {curatorName: '하티'});
                this.quratorId == '7';
            this.$router.push('/qurator/7');
        }else if(i == "해피소민") {
                this.$amplitude.getInstance().logEvent("click curatorThumbnail", {curatorName: '해피소민'});
                this.quratorId == '8';
            this.$router.push('/qurator/8');
        } else if(i == "민또링") {
                this.$amplitude.getInstance().logEvent("click curatorThumbnail", {curatorName: '민또링'});
                this.quratorId == '9';
            this.$router.push('/qurator/9');
        } else if(i == "리엔") {
                this.$amplitude.getInstance().logEvent("click curatorThumbnail", {curatorName: '리엔'});
                this.quratorId == '10';
            this.$router.push('/qurator/9');
        } else {
            console.log('이건 가계정');
        ...
        // switch(i) {
        //     case '민송' :
        //         this.$amplitude.getInstance().logEvent("click curatorProfile", {curatorName: '민송'});
        //         this.quratorId == '6';
        //     break;
        //     case '하티' :
        //         this.$amplitude.getInstance().logEvent("click curatorProfile", {curatorName: '하티'});
        //         this.quratorId == '7';
        //     break;
        //     case '해피소민' :
        //         this.$amplitude.getInstance().logEvent("click curatorProfile", {curatorName: '해피소민'});
        //         this.quratorId == '8';
        //     break;
        //     default: 
        //         this.quratorId == '999';
        // }
        // if(this.quratorId = '999') {
        //     console.log('이건 가계정');
        // } else {
        //     this.$router.push('/qurator/'+this.quratorId);
        // }
    },
    ...
    modiTime(time) {        
        ...
        modiCount(cnt) {
            ...
            return cnt.substring(0, index) + "," + cnt.substring(index);
        },
    },
