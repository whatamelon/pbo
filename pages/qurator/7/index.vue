...
export default {
  ...
  computed: {
    ...
    async asyncData({ store }) {
        await store.dispatch("getQuratorInfo", '7');
        await store.dispatch("getQuratorQurationAPI", '7');
    },
    ...
    goToSNS(i, link) {
        if(i == 'inst') {
            this.$amplitude.getInstance().logEvent("click sns", {curatorName: '하티', snsKind:'인스타그램'});
            window.open("https://www.instagram.com/"+link+"/");
        } else {
            this.$amplitude.getInstance().logEvent("click sns", {curatorName: '하티', snsKind:'유튜브'});
